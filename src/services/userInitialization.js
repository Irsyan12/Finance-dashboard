...
export const createDefaultCategories = async (userId) => {
    try {
        ...
        // Try batch insert first
        try {
            const batchResult = await categoryService.createBatch(categoriesWithUserId);
            return batchResult;
        } catch (batchError) {
            ...
            for (let i = 0; i < categoriesWithUserId.length; i++) {
                ...
                try {
                    console.log(
                        `🔄 Creating category ${i + 1}/${categoriesWithUserId.length}: ${categoryData.name} (${categoryData.type})`
                    );
                    const category = await categoryService.create(categoryData);
                    createdCategories.push(category);
                    console.log(
                        `✅ Successfully created: ${categoryData.name} (ID: ${category?.id})`
                    );
            ...
            console.log(
                `✅ Successfully created ${createdCategories.length}/${DEFAULT_CATEGORIES.length} default categories`
            );
...
export const debugCategoryCreation = async (userId) => {
    try {
        ...
        const existingCategories = await categoryService.getByUserId(userId);
        console.log(
            `✅ Can read categories table. Found ${existingCategories.length} existing categories`
        );

        // Test 2: Try creating a single test category
        console.log("Test 2: Testing single category creation...");
        const testCategory = {
        ...
        console.log("✅ Single category creation successful:", created);

        // Test 3: Clean up test category
        if (created?.id) {
            try {
                await categoryService.delete(created.id);
                console.log("✅ Test category cleanup successful");
            } catch (error) {
...
export const initializeUserCategories = async (userId) => {
    try {
        ...
        if (success) {
            console.log(
                `✅ User initialization complete: created ${createdCategories.length} categories`
            );
            return true;
        } else {
            console.warn("⚠️ User initialization failed: no categories created");
            return false;
        }
    } catch (error) {
        throw error;
